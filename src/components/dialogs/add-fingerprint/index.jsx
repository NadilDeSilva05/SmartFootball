...
const AddFingerprintModal = ({ open, setOpen, data, onCancelEnroll }) => {
  ...
  const onCancelEnrollRef = useRef(onCancelEnroll)
  ...
  useEffect(() => {
    ...
    onCancelEnrollRef.current = onCancelEnroll
  }, [setOpen, onCancelEnroll])
  ...
  const handleClose = useCallback(() => {
    ...
    // Use ref to ensure we always have the latest setOpen function
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      console.log('Fingerprint enrollment successful:', result)
      setSuccessMessage('Fingerprint added successfully!')
      setSuccess(true)
      ...
      closeTimeoutRef.current = setTimeout(() => {
        // Use refs to ensure we have the latest callbacks
        handleClose()
        // Refresh client data after successful enrollment
        if (onCancelEnrollRef.current) {
          onCancelEnrollRef.current()
        }
        closeTimeoutRef.current = null
      }, 2000)
    } catch (err) {
      console.error('Error enrolling fingerprint:', err)
      setError(err.message || 'Failed to add fingerprint')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleCancelEnroll = async () => {
    ...
    try {
      const response = await fetch('/api/fingerprint/cancel-enroll', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          clientId: data.clientId,
      ...
      console.log('Fingerprint enrollment cancelled successfully:', result)
      setSuccessMessage('Fingerprint enrollment cancelled successfully!')
      setSuccess(true)
      ...
      closeTimeoutRef.current = setTimeout(() => {
        // Use refs to ensure we have the latest callbacks
        handleClose()
        // Refresh client data after canceling enrollment
        if (onCancelEnrollRef.current) {
          onCancelEnrollRef.current()
        }
        closeTimeoutRef.current = null
      ...
      console.error('Error cancelling fingerprint enrollment:', err)
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='md' fullWidth>
      <DialogTitle
        variant='h4'
        className='flex flex-col gap-2 text-center pbs-10 pbe-6'
        sx={{
          px: { xs: 2, sm: 4 },
          pt: { xs: 2.5, sm: 4 },
          pb: { xs: 1.5, sm: 3 }
      ...
      >
        ...
        <Typography component='span' className='flex flex-col text-center'>
          Register a new fingerprint for {data?.clientName || 'client'}
        </Typography>
      </DialogTitle>

      <form onSubmit={handleSubmit}>
        <DialogContent 
          className='overflow-visible pbs-0 pbe-6'
          sx={{ 
            pl: { xs: 2, sm: 4 }, 
            pr: { xs: 2, sm: 4 },
            pb: { xs: 2, sm: 3 }
        ...
        >
          ...
          <Grid container spacing={{ xs: 3, sm: 5 }}>
            ...
            {success && (
              <Grid item size={12}>
                <Alert severity='success'>{successMessage || 'Operation completed successfully!'}</Alert>
              </Grid>
            )}
        ...
        >
          <Button
            variant='contained'
            type='submit'
            disabled={!selectedDevice || isSubmitting || isCancelling || !isConnected}
            startIcon={isSubmitting ? <CircularProgress size={16} /> : <i className='ri-fingerprint-line' />}
            fullWidth={isMobile}
            sx={{ minWidth: { xs: '100%', sm: 'auto' } }}
          >
            {isSubmitting ? 'Adding Fingerprint...' : 'Add Fingerprint'}
          </Button>
          <Button
            variant='outlined'
            color='error'
            ...
            disabled={!selectedDevice || isSubmitting || isCancelling || !isConnected}
            startIcon={isCancelling ? <CircularProgress size={16} /> : <i className='ri-close-circle-line' />}
            fullWidth={isMobile}
            sx={{ minWidth: { xs: '100%', sm: 'auto' } }}
          >
            {isCancelling ? 'Cancelling...' : 'Cancel Enroll'}
          </Button>
          <Button 
            variant='outlined' 
            color='secondary' 
